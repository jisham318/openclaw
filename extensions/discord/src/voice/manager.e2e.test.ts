...
describe("DiscordVoiceManager", () => {
  ...
  it("allows the same speaker to restart after finalize fires", async () => {
    ...
    try {
      ...
      const secondCallbacks: StreamCallbacks = {};
      const secondStreamDestroy = vi.fn(() => {
        for (const cb of secondCallbacks["close"] ?? []) cb();
      });
      const secondStream = {
        on: vi.fn((event: string, cb: (...args: unknown[]) => void) => {
          (secondCallbacks[event] ??= []).push(cb);
        }),
        destroy: secondStreamDestroy,
      };
      ...
      for (const cb of secondCallbacks["end"] ?? []) cb();
  ...
  it("force-cuts capture at maxSegmentDurationMs and re-triggers for the same speaker", async () => {
    vi.useFakeTimers();
    try {
      const connection = createConnectionMock();
      joinVoiceChannelMock.mockReturnValueOnce(connection);
      const manager = createManager({ voice: { maxSegmentDurationMs: 2_000 } });

      await manager.join({ guildId: "g1", channelId: "1001" });

      type StreamCallbacks = Record<string, ((...args: unknown[]) => void)[]>;
      const firstStreamCallbacks: StreamCallbacks = {};
      const firstStreamDestroy = vi.fn(() => {
        for (const cb of firstStreamCallbacks["close"] ?? []) cb();
      });
      const firstStream = {
        on: vi.fn((event: string, cb: (...args: unknown[]) => void) => {
          (firstStreamCallbacks[event] ??= []).push(cb);
        }),
        destroy: firstStreamDestroy,
      };

      const secondStreamCallbacks: StreamCallbacks = {};
      const secondStreamDestroy = vi.fn(() => {
        for (const cb of secondStreamCallbacks["close"] ?? []) cb();
      });
      const secondStream = {
        on: vi.fn((event: string, cb: (...args: unknown[]) => void) => {
          (secondStreamCallbacks[event] ??= []).push(cb);
        }),
        destroy: secondStreamDestroy,
      };
      ...
      expect(firstStreamDestroy).toHaveBeenCalledTimes(1);

      await capturePromise;

      expect(connection.receiver.subscribe).toHaveBeenCalledTimes(2);
      expect(connection.receiver.subscribe).toHaveBeenNthCalledWith(
        2,
        "u1",
        expect.objectContaining({ end: { behavior: "Manual" } }),
      );
  ...
  it("does not hard-cut when capture ends naturally before max duration", async () => {
    vi.useFakeTimers();
    try {
      const connection = createConnectionMock();
      joinVoiceChannelMock.mockReturnValueOnce(connection);
      const manager = createManager({ voice: { maxSegmentDurationMs: 5_000 } });

      await manager.join({ guildId: "g1", channelId: "1001" });

      type StreamCallbacks = Record<string, ((...args: unknown[]) => void)[]>;
      const callbacks: StreamCallbacks = {};
      const streamDestroy = vi.fn();
      const stream = {
        on: vi.fn((event: string, cb: (...args: unknown[]) => void) => {
      ...
      for (const cb of callbacks["end"] ?? []) cb();
